...
export default function DashboardHome({ onNavigate }) {
      ...
      const cards = [
            { key: 'total', label: 'Total Complaints', value: stats.total, icon: FileText, gradient: 'from-blue-500 to-blue-600' },
            { key: 'pending', label: 'Pending', value: stats.pending, icon: Clock, gradient: 'from-amber-500 to-orange-500' },
            { key: 'inProgress', label: 'In Progress', value: stats.inProgress, icon: AlertTriangle, gradient: 'from-violet-500 to-purple-600' },
            { key: 'resolved', label: 'Resolved', value: stats.resolved, icon: CheckCircle2, gradient: 'from-emerald-500 to-teal-500' },
            { key: 'emergency', label: 'Emergency', value: stats.emergency, icon: Siren, gradient: 'from-red-500 to-rose-600' },
      ];

      return (
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                  <motion.div>
                        <h1 className="text-2xl font-black text-slate-900">Dashboard Home</h1>
                        <p className="text-slate-500 text-sm">AI-powered grievance overview</p>
                  </motion.div>
                  ...
                  <div className="grid lg:grid-cols-3 gap-6">
                        <motion.div className="lg:col-span-2 glass rounded-2xl border border-white/60 p-5 shadow-sm">
                              ...
                              {recent.length === 0 ? (
                                    <p className="text-sm text-slate-500 py-8 text-center">No complaints yet. File your first complaint!</p>
                              ) : (
                        ...
                        <motion.div className="glass rounded-2xl border border-indigo-100 bg-gradient-to-br from-indigo-50 to-violet-50 p-5 shadow-sm">
                              <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <h2 className="font-bold text-indigo-900">AI Insights</h2>
                              </div>
                              <ul className="text-xs text-indigo-800 space-y-2">
                                    <li>• Most issues resolve within 5–7 working days</li>
                                    <li>• Use emergency mode for crime, fire, or medical crises</li>
                                    <li>• AI auto-routes complaints to the right department</li>
                                    <li>• Upload clear photos for faster resolution</li>
                              </ul>
                              <button
                                    type="button"
                              ...
                              >
                                    File Complaint <ChevronRight className="w-4 h-4" />
                              </button>
                        </motion.div>
                  </div>
            </motion.div>
